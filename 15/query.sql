...
FROM
  mahasiswa
  LEFT JOIN nilai_mahasiswa USING(nim)
WHERE
  nilai <= "B";
...
FROM
  nilai_mahasiswa
  LEFT JOIN mahasiswa ON nilai_mahasiswa.nim = mahasiswa.nim
  LEFT JOIN matakuliah ON matakuliah.id_mk = nilai_mahasiswa.id_mk
GROUP BY
  nilai_mahasiswa.nim
HAVING
  SUM(sks) > 10;
...
SELECT
  dosen.nama,
  COUNT (DISTINCT mahasiswa.nim)
FROM
  nilai_mahasiswa
  LEFT JOIN dosen ON nilai_mahasiswa.id_dosen = dosen.id_dosen
  LEFT JOIN mahasiswa ON mahasiswa.nim = nilai_mahasiswa.nim
GROUP BY
  dosen.id_dosen;
...
FROM
  ...
  LEFT JOIN nilai_mahasiswa ON mahasiswa.nim = nilai_mahasiswa.nim
  LEFT JOIN dosen ON dosen.id_dosen = mahasiswa.id_dosen_wali
  LEFT JOIN matakuliah ON matakuliah.id_mk = nilai_mahasiswa.id_mk
WHERE
  nilai_mahasiswa.nilai = 'D'
  OR nilai_mahasiswa.nilai = 'E';